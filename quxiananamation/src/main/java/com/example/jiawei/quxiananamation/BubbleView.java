...
public class BubbleView extends RelativeLayout{
    ...
    private int riseDuration = 4000;//上升的时间

    private int bottomPadding = 200;
    private int originsOffset = 200;//什么东西

    private float maxScale = 1.0f;
    private float minScale = 1.0f;

    private int innerDelay = 100;//点击后的延时
    ...
    public void startAnimation(final int rankWidth, final int rankHeight){
        Observable.timer(innerDelay, TimeUnit.MILLISECONDS)
                .repeat(/*(int)(Math.random() * (maxHeartNum - minHeartNum)) + minHeartNum*/1)//点击一下 出来一个 不是多个
    ...
    private void bubbleAnimation(int rankWidth, int rankHeight){
        ...
        ObjectAnimator riseAlphaAnimator = ObjectAnimator.ofFloat(tempImageView, "alpha", 1.0f,0.8f,0.6f,0.4f,0.0f, 0.0f);//多加几个参数 提前变0
    ...
    private ValueAnimator getBesselAnimator(final ImageView imageView, int rankWidth, int rankHeight){
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                imageView.setTranslationX(currentPosition[0]/4);//让飘的时候 不平移那么远
